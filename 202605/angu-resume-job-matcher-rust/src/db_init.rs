fn init_db(conn: &Connection) -> Result<()> {
    ...
    let user_count: i64 = conn.query_row("SELECT COUNT(*) FROM sys_user", [], |r| r.get(0))?;
    if user_count > 0 {
        return Ok(());
    }

    let t = "2026-05-27 14:37:46";
    conn.execute("INSERT INTO sys_user(id,username,real_name,phone,password,status,error_count,deleted,created_time,updated_time) VALUES(1,'admin','System Admin','13800000000','$2a$10$CHHVPySZmO5DsDtGjAXwXugfDw4hPiI4HiOGoc8MSuyJqVM1e000q',1,0,0,?,?)", params![t, t])?;
    conn.execute("INSERT INTO sys_role(id,role_name,role_code,builtin,status,remark,created_time,updated_time) VALUES(1,'Super Admin','SUPER_ADMIN',1,1,'Built-in super admin',?,?),(2,'HR Admin','HR_ADMIN',0,1,'HR admin',?,?),(3,'HR Staff','HR_STAFF',0,1,'HR staff',?,?)", params![t,t,t,t,t,t])?;
    conn.execute("INSERT INTO sys_user_role(user_id,role_id) VALUES(1,1)", [])?;

    let menus = vec![
        (1, 0, 0, "System", "/system", None::<&str>, "setting", 1),
        (2, 0, 0, "Jobs", "/jobs", None::<&str>, "briefcase", 2),
        (3, 0, 0, "Resumes", "/resumes", None::<&str>, "file", 3),
        (4, 0, 0, "Match", "/match", None::<&str>, "search", 4),
        (
            5,
            0,
            0,
        ...
        (6, 0, 0, "Reports", "/stats", None::<&str>, "bar-chart", 6),
        (11, 1, 1, "Users", "/system/users", None::<&str>, "user", 1),
        (12, 1, 1, "Roles", "/system/roles", None::<&str>, "team", 2),
        (13, 1, 1, "Menus", "/system/menus", None::<&str>, "menu", 3),
        (
            21,
            2,
            1,
    ...
    for (id, p, tpe, name, path, perm, icon, sort) in menus {
        conn.execute("INSERT INTO sys_menu(id,parent_id,menu_type,menu_name,path,perm_code,icon,sort) VALUES(?,?,?,?,?,?,?,?)", params![id,p,tpe,name,path,perm,icon,sort])?;
    }
    let perms = vec![
        (1101, 11, "User List", "system:user:list", 1),
        (1102, 11, "User Add", "system:user:add", 2),
        (1103, 11, "User Edit", "system:user:edit", 3),
        (1104, 11, "User Delete", "system:user:delete", 4),
        (1201, 12, "Role List", "system:role:list", 1),
        (1202, 12, "Role Add", "system:role:add", 2),
        (1203, 12, "Role Edit", "system:role:edit", 3),
        (1204, 12, "Role Delete", "system:role:delete", 4),
        (1301, 13, "Menu List", "system:menu:list", 1),
        (1302, 13, "Menu Add", "system:menu:add", 2),
        (1303, 13, "Menu Edit", "system:menu:edit", 3),
        (1304, 13, "Menu Delete", "system:menu:delete", 4),
        (2101, 21, "Job List", "job:job:list", 1),
        (2102, 21, "Job Add", "job:job:add", 2),
        (2103, 21, "Job Edit", "job:job:edit", 3),
        (2104, 21, "Job Delete", "job:job:delete", 4),
        (2105, 21, "Job Publish", "job:job:publish", 5),
        (2106, 21, "Job Close", "job:job:close", 6),
        (3101, 31, "Resume List", "resume:resume:list", 1),
        (3102, 31, "Resume Add", "resume:resume:add", 2),
        (3103, 31, "Resume Edit", "resume:resume:edit", 3),
        (3104, 31, "Resume Delete", "resume:resume:delete", 4),
        (3105, 31, "Resume Export", "resume:resume:export", 5),
        (4101, 41, "Match Execute", "match:match:execute", 1),
        (4102, 41, "Match Config View", "match:config:view", 2),
        (4103, 41, "Match Config Edit", "match:config:edit", 3),
        (5101, 51, "Application List", "application:list", 1),
        (5102, 51, "Application Add", "application:add", 2),
        (5103, 51, "Application Edit", "application:edit", 3),
        (5201, 52, "Interview List", "interview:list", 1),
        (5202, 52, "Interview Add", "interview:add", 2),
        (5203, 52, "Interview Edit", "interview:edit", 3),
        (6101, 61, "Dashboard View", "stats:dashboard:view", 1),
        (6201, 62, "Source Report View", "stats:report:view", 1),
        (6202, 62, "Source Report Export", "stats:report:export", 2),
    ];
    for (id, p, name, perm, sort) in perms {
        conn.execute("INSERT INTO sys_menu(id,parent_id,menu_type,menu_name,path,perm_code,icon,sort) VALUES(?,?,2,?,NULL,?,NULL,?)", params![id,p,name,perm,sort])?;
    }
    let menu_ids = query_values(conn, "SELECT id FROM sys_menu", [], |r| r.get::<_, i64>(0))
        .map_err(|e| anyhow!(e.message))?;
    for id in menu_ids {
        conn.execute(
            "INSERT INTO sys_role_menu(role_id,menu_id) VALUES(1,?)",
            params![id],
        )?;
    }

    conn.execute("INSERT INTO job_position(id,title,department,job_type,location,salary_min,salary_max,edu_require,exp_require,description,skill_tags,status,deleted,creator_id,created_time,updated_time) VALUES(1,'自动化测试职位-0527','智能招聘部','FULL_TIME','杭州',10000,20000,'BACHELOR',1,'自动化测试创建的职位，用于验证前端主流程。','[\"Vue3\",\"TypeScript\",\"招聘系统\"]','OPEN',0,1,'2026-05-27 16:17:13','2026-05-27 16:18:52')", [])?;
    ...
    conn.execute("INSERT INTO match_config(id,skill_weight,edu_weight,exp_weight,updated_time,updater_id) VALUES(1,50,30,20,'2026-05-27 16:59:08',1)", [])?;
